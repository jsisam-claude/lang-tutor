...
| Download fails `SSLHandshakeException` | Network intercepts TLS (VPN/ad-blocker/WiFi filter). Use the debug **Ignore SSL** button, try mobile data, or use Option B. Note the button is debug-only — a **release** build has no such escape hatch and a user-installed CA is not trusted by the app either, so sideload the models instead (docs/building-on-debian.md §TLS) |
