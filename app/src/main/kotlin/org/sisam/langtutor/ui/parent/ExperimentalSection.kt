    ...
    Card(
        ...
        Column(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    Text(
                        text = stringResource(R.string.parent_voice_barge_title),
                        style = MaterialTheme.typography.titleSmall,
                    )
                    Text(
                        text = stringResource(R.string.parent_voice_barge_hint),
                        style = MaterialTheme.typography.bodySmall,
                    )
                }
                Switch(
                    checked = profile.parentSettings.tryVoiceBarge,
                    onCheckedChange = { on ->
                        scope.launch {
                            container.profile.update {
                                it.copy(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    Text(
                        text = stringResource(R.string.parent_streaming_asr_title),
                        style = MaterialTheme.typography.titleSmall,
                    )
                    Text(
                        text = stringResource(R.string.parent_streaming_asr_hint),
                        style = MaterialTheme.typography.bodySmall,
                    )
                }
                Switch(
                    checked = profile.parentSettings.tryStreamingAsr,
                    onCheckedChange = { on ->
                        scope.launch {
                            container.profile.update {
                                it.copy(
